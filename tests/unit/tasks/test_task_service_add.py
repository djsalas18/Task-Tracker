...
def test_add_task_creates_new_task(tmp_path):
    ...
    # Add a new task
    new_task = service.add_task("Buy groceries", "low", "Milk and eggs")
    ...
    second_task = service.add_task("Do homework", "medium")
...
def test_add_task_none_title():
    ...
    service = TaskService(storage=None)
    with pytest.raises(ValueError):
        service.add_task(None, "low","No title")

@pytest.mark.unit
def test_add_task_empty_priority():
    service = TaskService(storage=None)
    with pytest.raises(ValueError):
        service.add_task("A Title", "", "No title")

@pytest.mark.unit
def test_add_task_none_priority():
    service = TaskService(storage=None)
    with pytest.raises(ValueError):
        service.add_task("A Title", None,"No title")