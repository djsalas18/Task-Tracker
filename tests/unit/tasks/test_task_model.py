...
def test_task_instantiation():
    ...
    task = Task(task_id=1, title="Write tests", priority="low", description="Write unit tests for Task", completed=False)
    assert task.id == 1
...
def test_task_default_completed_false():
    ...
    task = Task(task_id=2, title="Do homework", priority="low", description="Math exercises")
