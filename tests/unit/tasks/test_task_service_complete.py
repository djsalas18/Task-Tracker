...
def test_complete_task_success():
    ...
    # Add a task first
    new_task = service.add_task("Test Task", "low", "Test description")
    task_id = new_task["id"]
    ...
    assert completed_task["priority"] == "low", "Priority should remain unchanged"
...
def test_complete_already_completed_task():
    ...
    new_task = service.add_task("Already done", "low", "Already completed task")
    task_id = new_task["id"]
