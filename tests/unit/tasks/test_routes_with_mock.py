...
def test_add_task_route_uses_service(app_with_mock):
    ...
    response = client.post('/api/tasks', json={"title": "Mock Task", "priority": "low", "description": "Test DI"})
...
def test_complete_task_route_with_mock(app_with_mock):
    ...
    response = client.post('/api/tasks', json={"title": "Finish Lab", "priority": "high", "description": "Complete Sprint 3 Lab 2"})
    assert response.status_code == 201
    t = response.get_json()
    task_id = t["id"]
...
def test_delete_task_route_with_mock(app_with_mock):
    ...
    resp1 = client.post('/api/tasks', json={"title": "Task A", "priority": "low"})
    assert resp1.status_code == 201
    resp2 = client.post('/api/tasks', json={"title": "Task B", "priority": "low"})
