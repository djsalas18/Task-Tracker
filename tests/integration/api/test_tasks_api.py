...
def test_add_and_get_tasks_end_to_end():
    ...
    # 1. Add a new task
    task_data = {"title": "Integration Test Task", "priority": "medium", "description": "End-to-end test via requests"}
...
def test_complete_task_end_to_end():
  ...
  resp = requests.post(BASE_URL, json={"title": "Task to complete", "priority": "medium"})
  assert resp.status_code == 201
  task = resp.json()
  task_id = task["id"]
...
def test_delete_task_end_to_end():
    ...
    resp = requests.post(BASE_URL, json={"title": "Task to delete", "priority": "medium"})
