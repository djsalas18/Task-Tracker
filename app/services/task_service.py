...
class TaskService:
    ...
    def get_all_tasks_sorted(self, reverse=False):
        """Get all tasks from storage (as dicts) (sorted)."""
        temp_list = []

        # Sort based on __eq__, __lt__, and __gt__ implementation in Task
        for task in sorted([t for t in self._tasks], reverse=reverse):
    ...
    def add_task(self, title, priority, description=None):
        ...
        # Validate priority, though this is not possible if done through the frontend
        if not priority or priority is None:
            raise ValueError("Priority cannot be empty or None")

        priority_string = str(priority).lower().strip()
        if priority_string not in ['low', 'medium', 'high']:
        ...
        new_task_obj = Task(
            next_id, title, priority, description if description is not None else "", False
        )
